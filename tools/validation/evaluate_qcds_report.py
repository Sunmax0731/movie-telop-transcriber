...
def build_report(
    ...
    lines = [
        ...
        f"  --output {output_path if output_path else 'docs/test-results/<report>.md'} `",
        f"  --metrics-output {metrics_output_path if metrics_output_path else 'docs/test-results/<metrics>.json'}",
        "```",
        "",
        "## 4. QCDSサマリ",
        "| 観点 | 指標 | 今回値 | 前回値 | 差分 |",
    ...
    lines.extend(
        [
            ...
            "",
            "## 10. 判定メモ",
            f"- `{sample_id}` の基準比較では、正解セグメントに対して文字列、時刻、件数を確認する。",
