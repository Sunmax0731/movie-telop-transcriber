...
def build_report(
    ...
    lines = [
        ...
        "- この sidecar 評価スクリプトでは OCR 矩形から `font_size` を推定し、`font_family`、色、背景色、厳密な種別は未推定値として扱う。",
    ...
    lines.extend(
        [
            ...
            "- この sidecar 評価スクリプトでは OCR 矩形からの高さ推定に限られるため、フォント名、文字色、枠色、背景色、装飾種別の実用精度評価は未成立。",
            ...
            "- `font_family`、`text_color`、`stroke_color`、`background_color`、精密な `text_type` は、この sidecar 評価スクリプトだけでは算出しない。",
