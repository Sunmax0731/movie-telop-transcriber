...
4. 現在の個別 Issue: `#90` QCDS 評価用の代表動画セットと計測レポート整備
...
- 現行の OCR 精度検証では、PaddleOCR が既定の実 OCR 経路であり、Python ランタイムや PaddleOCR が未構成の場合は OCR エラーとして表示する。サンプル sidecar では文字列とセグメント時刻が期待値と一致する一方、フォント名、色、背景色、精密な種別は未推定値として扱う。
