...
- `#86` でタイムライン選択中テロップの結合 / 分割を追加し、編集操作の履歴を `segments.json` の `edits` に出力する。
...
4. 現在の個別 Issue: `#86` OCR 結果の編集・結合・分割 UI
...
- タイムライン上のテロップは選択後にコピー、編集、結合、分割、削除できる。編集内容は画面上の現在結果へ反映し、ファイルへ反映する場合は `出力のみ` を実行する。
- `segments.json` には GUI 上の編集、結合、分割、削除の履歴を `edits` として保存し、元 OCR / セグメント結果との追跡に使えるようにする。
