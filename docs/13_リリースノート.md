...
## 2. v0.1.2 リリース概要
`v0.1.2` は、`v0.1.1` の導入インストーラを一般利用者向けに仕上げるための配布改善リリースである。
...
- `Uninstall-MovieTelopTranscriber.ps1` / `.cmd` を追加し、導入先フォルダ、スタートメニューショートカット、導入先を指すユーザー環境変数、導入で新規取得したモデルを削除できるようにした。
- `README.md` と `docs/12_導入手順書.md` を、配布 zip を展開した一般利用者がそのまま辿れる導線へ更新した。

## 3. v0.1.2 配布物
GitHub Release `v0.1.2` では、次の asset を公開対象とする。

```text
movie-telop-transcriber-win-x64-v0.1.2.zip
movie-telop-transcriber-win-x64-v0.1.2.zip.sha256
...
## 9. 次回パッチリリースで反映予定の修正
- `#125` 配布 zip 側 `app` から起動した場合の `settings.json` fallback と、PaddleOCR `lang=ja` 正規化を反映する。
- `#127` インストーラを起動したディレクトリに、導入後アプリを指す `Movie Telop Transcriber.lnk` を作成する。
- `#127` アンインストーラで、導入先、スタートメニューショートカット、インストーラ実行ディレクトリの起動ショートカット、導入時に新規作成した外部 `ocr-runtime` を削除する。
