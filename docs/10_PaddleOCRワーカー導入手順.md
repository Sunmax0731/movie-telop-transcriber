...
初回実行時に `PP-OCRv5_server_det` と `PP-OCRv5_server_rec` がユーザープロファイル配下へダウンロードされる。#48 では、初期リリースのアプリ本体 zip に Python runtime、PaddlePaddle / PaddleOCR の Python package、PaddleOCR モデル本体を同梱しない方針にした。オフライン導入時の事前配置手順は #49 で整理する。
...
## 9. 既知制約
- PaddleOCR の Python runtime、Python package、モデル本体は、初期リリースのアプリ本体 zip には同梱しない。
- 初回モデル取得にはネットワーク接続が必要になる。オフライン導入では、#49 の手順に従ってモデルと Python 環境を事前配置する。
...
- 出力属性の `font_size` は OCR 矩形高さ相当であり、実フォントサイズではない。`text_color`、`stroke_color`、`text_type` は色/枠の暫定ラベルであり、厳密な色値や意味分類ではない。
- `font_family` と `background_color` は初期リリースでは `null` を許容する。SRT / VTT はテキストと時刻のみ、ASS は標準スタイルのみを使う。
- 大量フレームの速度最適化、モデル同梱、ONNX Runtime 化は #49 以降で継続判断する。
