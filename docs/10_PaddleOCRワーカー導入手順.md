...
| `MOVIE_TELOP_PADDLEOCR_UPSCALE` | OCR 前に画像全体を拡大する倍率。 | `1.5` |
...
これらのうち、前処理、検出閾値、文字方向分類、文書ゆがみ補正は設定画面からも変更できる。変更後に OCR を再実行すると、`PaddleOcrWorkerClient` は設定差分を検出して worker を再起動する。
...
前処理 ON (`upscale=1.5`、`contrast=1.1`、`sharpen=true`) でも同一フレームから以下を検出した。
...
- 前処理の拡大率やコントラストは動画によって認識結果と処理時間に影響する。検出が悪化する場合は設定画面または環境変数で `MOVIE_TELOP_PADDLEOCR_PREPROCESS=false`、または拡大率を `1.0` に調整する。
