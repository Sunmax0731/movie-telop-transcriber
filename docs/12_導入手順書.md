...
| 構成 | 想定利用者 | 利点 | 欠点 | 判断 |
| --- | --- | --- | --- | --- |
| オンライン installer 標準導入 | 一般利用者 | app / runtime / settings を一括で揃えられる | ネットワーク接続と Python 3.10-3.13 (64-bit) が必要 | 標準採用 |
...
配布 zip を展開した直後は、zip 直下の `START_HERE.md` を入口として参照する。
...
| 項目 | 値 |
| --- | --- |
| Python | `3.10.x` / `3.11.x` / `3.12.x` / `3.13.x` (64-bit) |
...
3. Python 3.10-3.13 (64-bit) の仮想環境を作成する。
4. `paddlepaddle==3.2.2` と `paddleocr==3.5.0` を導入する。
...
py -3.12 -m venv D:\Tools\movie-telop-ocr-runtime\.venv

D:\Tools\movie-telop-ocr-runtime\.venv\Scripts\python.exe -m pip install --upgrade pip
D:\Tools\movie-telop-ocr-runtime\.venv\Scripts\python.exe -m pip install paddlepaddle==3.2.2
...
| インストーラが `py -3.13` / `py -3.12` / `py -3.11` / `py -3.10` を見つけられない | Python Launcher と Python 3.10-3.13 (64-bit) が導入されているか | Python 3.10-3.13 (64-bit) を導入するか、`-PythonCommand` を指定する |
