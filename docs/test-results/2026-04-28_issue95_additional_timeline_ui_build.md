...
- 処理状況ペインの下部に再実行ボタン 3 つを横並びで配置し、直近の失敗は状態セクションへ移動する。
- 手動確認フィードバックを受け、選択テキストコピー、列表示 ON/OFF、進捗ラベルの多言語化と毎秒更新、二重丸形状の `◎` 補完、テロップ種類ラベル、設定画面タイトル削除を追加する。
- 追加フィードバックを受け、タイムライン詳細列を表示属性の一行表示に整理し、設定画面の PaddleOCR 固定日本語ラベルを多言語化する。
...
- `MainPage.xaml` / `SettingsWindow.xaml` の固定日本語表示を見直し、設定画面の検出設定ラベルとツールチップが `LocalizedUiText` 経由になったことを確認した。
...
- タイムライン詳細列の見出しが表示属性として表示され、同じ内容が二行表示されないこと。
...
- 設定画面の PaddleOCR 前処理、検出設定、各ツールチップが言語設定に合わせて切り替わること。
- 処理状況ペイン内で再実行ボタン 3 つが横並びで表示されること。
- 直近の失敗が左ペインの状態セクションに表示されること。
